...
Updated by Charlie Childs 2022 to make PDF versions of Cell Scoring Violin Plots
...
class sca_run:
	...
	def plot_sca(self, adata, figdir='./figures/', adata_preFiltered=None,
		...
		if self.umap_feature_color=='blue_orange':
			...
			feature_colors = [(210,210,210), (210,210,210), (255,255,0), (100,200,225), (0,45,125)]
		...
		## Check to see if user specified a color palette for categorical umap plots, ie. louvain, obs_fields
			...
			#colors = ['#ff0000','#b20000', '#800080', '#ffff00', '#4c0000', '#4c004c', '#ffe5e5', '#ff6666', '#e60000', '#FFA500', '#ff9999', '#ffc04c','#421010', '#b266b2', '#190000','#FFFF99']
#['#ff0000','#cc0000','#800080','#FFFF00','#7f0000','#b266b2','#ffb2b2','#660000','#190000','#FFA500','#ff3232','#cc8400','#0000FF','#ffd27f','#000000','#b2b200']
#'0','1','4','6','7','8','10','14','3','15','9','11','2','5','13','12'
#yellow='#ffff00', '#FFFF99'
#blue='#0000ff',
#red='#b20000', '#4c0000', '#ffe5e5', '#ff6666', '#e60000', #ff9999
#purple= '#800080',#4c004c, '#b266b2'
#orange= '#FFA500', '#ffc04c',
				...
				if len(gene_dict['markers'])!=0:
					for grouping in self.exp_grouping:
						...
						sc.pl.dotplot(adata_plots, genes_to_plot, groupby=grouping, 
								...
								color_map=my_feature_cmap, use_raw=True, dendrogram=dendrogram, return_fig=True).style(cmap=my_feature_cmap, grid=True).savefig(''.join(['_markers_',gene_list,'_',grouping,file_type]))#, figsize=(4,6))#, dot_max=0.4)#, dendrogram=True)
						## Heatmaps
		...
		if self.cell_score_lists:
			...
			for i, score_name in enumerate(self.cell_score_lists):
				sc.pl.umap(adata, color=score_name, 
						   save=''.join(['_',score_name,'_cellType_score.pdf']), show=False, edges=False, color_map=my_feature_cmap, 
						   size=size, vmin=vmin[i], vmax=vmax[i])
				sc.pl.umap(adata, color=score_name, 
						   save=''.join(['_',score_name,'_cellType_score_0min.pdf']), show=False, edges=False, color_map=my_feature_cmap, 
						   size=size, vmin=0, vmax=vmax[i])
				sc.pl.violin(adata, self.cell_score_lists, groupby='sampleName', jitter=0.4, save='_cell_scores.pdf', show=False, multi_panel=False, rotation=90)
